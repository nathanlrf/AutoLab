...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    numDiscStates = (int_T)mxGetNumberOfElements(DENOMINATOR_PARAM(S)) - 1;
...
static void mdlInitializeConditions(SimStruct *S)
{
    ...
    /* squirrel away the initial numerator and denominator */
    den0 = denParam[0];
    for (eIdx = 0; eIdx < mxGetNumberOfElements(DENOMINATOR_PARAM(S)); eIdx++) {
        *den++ = *denParam++ / den0;
    }
 
    num += ssGetNumDiscStates(S)+1-mxGetNumberOfElements(NUMERATOR_PARAM(S));

    for (eIdx = 0; eIdx < mxGetNumberOfElements(NUMERATOR_PARAM(S)); eIdx++) {
        *num++ = *numParam++ / den0;
    }
